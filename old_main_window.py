...
from third_party import AbstractFunctor, basename_decorator, AbstractCheckBoxList, \
    get_num_file_by_default, ItemWidget, ItemListWidget
import config as cf
...
class AbstractGraphWindowWidget(QWidget):
    def __init__(self, main_window_: MainWindow):
        ...
        self.sections_action = file_menu.addAction("&Настроить секции", "Ctrl+a")
        self.sections_action.triggered.connect(self.set_sections_action)
        ...
        self.section_dialog = SectionsDialog()
...
class ProcessingWidget(QPushButton):
    def __init__(self, window_size_: QSize, widget_owner_: QWidget):
        super().__init__("Processing...", widget_owner_)
        self.setEnabled(False)
        size = QSize(180, 50)
        position = QPoint(int(window_size_.width() - size.width() / 2), int(window_size_.width() - size.width() / 2))
...
class FileWidget(QWidget):
    def __init__(self, filename_: str, parent_: QWidget = None):
        ...
        self.label = QLabel(os.path.basename(self.filename), self)
        ...
        self.all_widget_to_layout()

    def __eq__(self, other_) -> bool:
        return self.filename == other_.filename

    def recreate(self, filename_: str) -> None:
        self.filename = filename_
        self.label.setText(os.path.basename(self.filename))
        self.setVisible(True)

    def all_widget_to_layout(self) -> None:
        core_layout = QHBoxLayout()
        core_layout.addWidget(self.label)
...
class FileListWidget(QWidget):
    ...
    def add_file(self, filename_: str, is_show: bool = False) -> None:
        for file in self.file_list:
            if file.filename is not None and file.filename == filename_:
                return
        for file in self.file_list:
            if file.filename is None:
                file.recreate(filename_)
                file.setVisible(is_show)
                return
        new_file = FileWidget(filename_, self)
        new_file.setVisible(is_show)
        self.file_list.append(new_file)
        self.layout().addWidget(new_file)


class SectionWidget(QWidget):
    def __init__(self, name_: str = '', hight_: int = 0, parent_: QWidget = None):
        super().__init__(parent_)
        self.name = name_
        self.hight = hight_
        ...
        self.hight_editor = QLineEdit(self)
        self.__editor_init()
        self.__values_to_editors()

        self.add_button = QPushButton('+', self)
        self.drop_button = QPushButton('▽', self)
        self.plot_button = QPushButton('P', self)
        self.delete_button = QPushButton('X', self)
        self.__button_init()
    ...
    def __editor_init(self) -> None:
        ...
        self.core_layout.addLayout(flo)

        flo = QFormLayout()
        self.hight_editor.setAlignment(Qt.AlignRight)
        self.hight_editor.textChanged.connect(self.hight_edit_action)
        flo.addRow("Глубина (м)", self.hight_editor)
        self.core_layout.addLayout(flo)

    def __button_init(self) -> None:
        self.add_button.setMaximumWidth(20)
        self.add_button.clicked.connect(self.add_files_action)
        self.core_layout.addWidget(self.add_button)

        self.drop_button.setMaximumWidth(20)
        self.drop_button.clicked.connect(self.drop_list_action)
        self.core_layout.addWidget(self.drop_button)

        self.plot_button.setMaximumWidth(20)
        self.plot_button.clicked.connect(self.plot_action)
        self.core_layout.addWidget(self.plot_button)

        self.delete_button.setMaximumWidth(20)
        self.delete_button.clicked.connect(self.delete_action)
        self.core_layout.addWidget(self.delete_button)

    def __values_to_editors(self) -> None:
        self.name_editor.setText(self.name)
        self.hight_editor.setText(str(self.hight))

    def recreate(self, name_: str = '', hight_: int = 0) -> None:
        self.name = name_
        self.hight = hight_
        self.__values_to_editors()
        self.setVisible(True)
    ...
    def hight_edit_action(self, text_: str) -> None:
        self.hight = 0 if len(text_) < 1 else int(text_)

    def add_files_action(self) -> None:
        file_list = select_files(self, cf.FILE_DIALOG_CSV_FILTER)
        for filename in file_list:
            self.file_list.add_file(filename, self.is_dropped)
...
class SectionListWidget(QWidget):
    ...
    def length(self) -> int:
        l = 0
        for file in self.section_list:
            if file.filename is not None:
                l += 1
        return l

    def add_section(self, name_: str = '', hight_: int = 0, is_show: bool = True) -> None:
        for section in self.section_list:
            if section.name is None:
                section.recreate(name_, hight_)
                section.setVisible(is_show)
                return
        new_section = SectionWidget(name_, hight_, self)
        new_section.setVisible(is_show)
        self.section_list.append(new_section)
        self.layout().addWidget(new_section)


class SectionsDialog(QDialog):
    def __init__(self):
        super().__init__()
        self.sections_list = SectionListWidget(self)

        self.add_button = QPushButton("+ Добавить", self)
        self.add_button.clicked.connect(self.add_action)

        self.plot_all_button = QPushButton("Построить для всех", self)
        self.plot_all_button.clicked.connect(self.plot_all_action)
    ...
    def all_widgets_to_layout(self) -> None:
        ...
        core_layout.addWidget(self.sections_list)

        core_layout.addWidget(self.add_button)
        core_layout.addWidget(self.plot_all_button)
    ...
    def add_action(self) -> None:
        self.sections_list.add_section()

    def plot_all_action(self) -> None:
        self.close()
    ...
    def read_csv_into_data_frame(self) -> None:
        ...
        for sensor_num in sensor_dict.keys():
            self.data_frames.append(Max1SensorDataFrame("sensor=" + str(sensor_num),
                                                        sensor_dict[sensor_num], self, cf.F4T44_X_AXES_MODE))
